...
const compress = (inputPath, outputPath) => {
    ...
    const windowSize   = 32768;  // was 32 — now matches DEFLATE standard
    const lookAhead    = 258;    // was 32
    const minMatchLen  = 3;

    // Issue 7 fix: hash chain for O(1) average match-finding
    // Maps a 3-byte hash → most recent position in the window
    const hashChain = new Map();   // hash  → head index
    const prev      = new Int32Array(bytes.length).fill(-1); // index → prev match
...
function buildHuffmanTrees(tokens) {
    ...
    for (const token of tokens) {
        ...
        // 'eof' token already seeded above — no extra counting needed
    }
    ...
    function buildTree(freqMap) {
        ...
        // Bug 2 fix: single-symbol case — return the leaf itself as root
...
function writeCompressedFile(outputPath, tokens, trees) {
    ...
    // frequencies — this makes decompression fully deterministic regardless
    ...
    // Decompressor reads these directly — no tree rebuild needed, no
    // tie-breaking ambiguity.
    const headerObject = {
        literalCodes,   // { symbol → bitstring }
        lengthCodes,
        distanceCodes,
